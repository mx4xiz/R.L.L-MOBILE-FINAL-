  ...
  return (
    <div className={`glass-panel rounded border transition-all duration-500 ${styles.bg} ${styles.border} ${styles.glow} group/quest hover:-translate-y-0.5 mb-3 md:mb-4 ${isProcessing ? 'opacity-70 pointer-events-none' : ''}`}>
        ...
        {isExpanded && quest.description && (
            <div className="px-4 md:px-5 pb-3 md:pb-4 pt-2 md:pt-3 border-t border-white/5 bg-black/40">
                <p className="text-[10px] md:text-xs text-gray-500 leading-normal uppercase tracking-widest italic font-medium">"{quest.description}"</p>
            </div>
        )}
    </div>
  );
