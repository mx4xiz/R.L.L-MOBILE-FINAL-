...
import { XP_PER_DIFFICULTY, getXpToNextLevel, getRankForLevel, TRAINING_PER_HALF_STAR, SKILL_UNLOCK_REQUIREMENTS, XP_FOR_SKILL_UNLOCK, STAT_POINTS_PER_DIFFICULTY, DUNGEONS, SYSTEM_QUESTS, XP_FOR_SKILL_ASCENSION, QUEST_COIN_REWARDS, ACHIEVEMENTS_DATA, MATERIALS, ADVANCEMENT_TRAITS, X_RANK_PENALTY_OVERRIDE, DAILY_XP_GOAL, ENHANCEMENT_REQUIREMENT } from '../constants';
...
export const usePlayerData = () => {
    ...
    const completeQuest = useCallback((id: string) => {
        ...
        setState(prevState => {
            ...
            return { 
                ...
                player: { 
                    ...prevState.player, 
                    shopCoins: prevState.player.shopCoins + (QUEST_COIN_REWARDS[q.difficulty] || 0), 
        ...
        if (QUEST_COIN_REWARDS[q.difficulty] > 0) {
            addNotification('REWARD GRANTED', `Received ${QUEST_COIN_REWARDS[q.difficulty]} Shop Coins.`, 'info');
        }
        // Release the lock
    ...
    const clearActiveDungeon = useCallback(() => {
        ...
        setState(prevState => {
            ...
            return { 
                ...
                dungeonCooldowns: { ...prevState.dungeonCooldowns, [dungeon.id]: { date: new Date().toISOString().split('T')[0], status: 'cleared' } }, 
    ...
    const failActiveDungeon = useCallback(() => {
        ...
        setState(prevState => ({ 
            ...
            dungeonCooldowns: { ...prevState.dungeonCooldowns, [dungeon.id]: { date: new Date().toISOString().split('T')[0], status: 'failed' } } 
        }));
    ...
    const exportState = useCallback(() => {
        ...
        downloadAnchorNode.setAttribute("download", `rll_backup_${new Date().toISOString().split('T')[0]}.json`);
